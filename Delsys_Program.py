...
class Controller(threading.Thread):
    ...
    def run(self):
        ...
        while (time.time() - init_time) < self.run_duration:
            try:
                read_out = self.sock.recv(100).decode()  # Note: this code and the timeout are a very messy way to do this. We will eventually use selectors
...
def main():
    ...
    test_class = Controller(('localhost', 50040), 15, True)
    test_class.start()
    test_class.join()
    sys.exit(0)
